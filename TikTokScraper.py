...
class TikTokScraper:
    ...
    # -----------------------------
    def get_transcript_urls_by_language(self,
                                        info: Dict[str, Any],
                                        *,
    ...
    def extract_playlist_with_transcripts(
        ...
        for e in entries:
            ...
            # structured lines for the chosen language
            e["transcript_lines"] = []

            # optional per-language structured lines
            e["transcript_lines_by_lang"] = {} if store_all_languages else None

            # per-language plain-text map
            e["transcripts"] = {} if store_all_languages else None

            captions = self.get_transcript_urls_by_language(e, prefer_requested=False, exts={"vtt", "srt"}, proxy_address=proxy)
            if not captions:
                continue
